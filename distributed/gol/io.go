...
func (io *ioState) readPgmImage() {
	...
	data_string := *(*string)(unsafe.Pointer(&data))
	util.Check(ioError)

	start_index := 0
	var fields [4]string
	fields_index := 0
	for i := 0; i != len(data_string) && fields_index != 4; i++ {
		switch data_string[i] {
		case '\n':
			fallthrough
		case ' ':
			fields[fields_index] = data_string[start_index:i]
			fields_index++
			start_index = i + 1
		}
	}
